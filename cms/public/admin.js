...
const ancestryChevron='<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg>';
function ancestorFields(nodes,key,depth=1){
  return `<div class="ancestor-generation ancestor-generation--${depth}">${nodes.map((node,index)=>{
    const path=`${key}.${index}`, role=index?'Мать':'Отец';
    return `<fieldset class="ancestor-fields"><legend>${role}</legend>${field('Кличка',path+'.name',node.name,'text','maxlength="120"')}${field('Титулы (необязательно)',path+'.title',node.title,'text','maxlength="500"')}${depth<3?`<details class="ancestor-more"><summary><span>Родители ${index?'матери':'отца'}<small>${depth+1}-е поколение</small></span>${ancestryChevron}</summary>${ancestorFields(node.parents,path+'.parents',depth+1)}</details>`:''}</fieldset>`;
  }).join('')}</div>`;
}
function pedigreeEditor(parent,key){
  parent.pedigreeTree=editablePedigree(resolvePedigree(parent));
  return `<div class="pedigree-editor"><h3>Родословная</h3><p class="help">Предки ${parent.role==='Мама'?'мамы':'папы'} помёта. Начните с отца и матери, затем раскройте следующие поколения. Заполняйте только известные данные.</p>${ancestorFields(parent.pedigreeTree,key+'.pedigreeTree')}${parent.pedigree?area('Примечание к родословной',key+'.pedigree',parent.pedigree,5000):''}</div>`;
}
