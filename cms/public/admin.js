...
function parentEditor(parent,i){const key=`parents.${i}`,known=new Map();for(const row of records.filter(r=>r.kind==='litters'))for(const p of row.data.parents)if(p.image)known.set(p.id,p);
  return `<section class="editor-section"><div class="section-title"><h2>${parent.role}</h2><button type="button" class="quiet" data-remove-parent="${i}">Убрать родителя</button></div><label>Взять данные из другого помёта<select data-parent-source="${i}"><option value="">Заполнить вручную</option>${[...known.values()].map(p=>`<option value="${e(p.id)}">${e(p.name)} · ${e(p.kennel)}</option>`).join('')}</select></label><div class="two-columns">${field('Кличка',key+'.name',parent.name,'text','maxlength="120"')}${field('Питомник происхождения',key+'.kennel',parent.kennel,'text','maxlength="160"')}</div>${photoEditor(key+'.image',parent.image?[{src:parent.image,width:parent.width,height:parent.height,alt:parent.name}]:[],true)}${area('Достижения и описание',key+'.description',parent.description)}${area('Родословная',key+'.pedigree',parent.pedigree,5000)}<p class="help">Для Эдель и Енота сохраняется текущая родословная, если это поле пустое.</p></section>`;}
...
app.addEventListener('change',async event=>{const input=event.target;
  if(input.dataset.parentSource!==undefined){const index=+input.dataset.parentSource,p=records.filter(r=>r.kind==='litters').flatMap(r=>r.data.parents).find(p=>p.id===input.value);if(p){const role=current.data.parents[index].role;current.data.parents[index]={...clone(p),role};dirty=true;editor();}return;}
