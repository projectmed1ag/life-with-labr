...
const routeFor = page => page.key === 'home' ? '/' : `/${page.key}/`;
for (const page of pages) {
  const route = routeFor(page);
  const links = list => list.map(item => `<a href="${routeFor(item)}"${item.key === page.key ? ' aria-current="page"' : ''}>${item.label}</a>`).join('');
  const values = {
    title: page.title, description: page.description, page:page.key, brand,
    canonical: new URL(route, origin).href, routeJson: JSON.stringify(route),
...
await writeFile('dist/sitemap.xml', `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${pages.map(page => `<url><loc>${new URL(routeFor(page), origin).href}</loc></url>`).join('')}</urlset>\n`);
