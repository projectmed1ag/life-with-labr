...
const fps=30;
const loopSeconds=8;
const halfIntervals=loopSeconds*fps/2;
const forwardCount=halfIntervals+1;
...
try {
  for(const {source,name,start,end,speed,first,count} of [
    ...
    // Retime the same moving segment to four seconds in each direction.
    // Calculate intermediate frames from the original source at the slower
    // speed, so the browser still plays a continuous 30 fps video at 1x.
    const stretch=halfIntervals/(count-1);
    const firstFrame=Math.round(first*stretch);
    const forward=join(folder,`${name}.mkv`);
    run(['-i',source,'-vf',
      `trim=start_frame=${start}:end_frame=${end},setpts=${speed*stretch}*(PTS-STARTPTS),`
      +`minterpolate=fps=${fps}:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1:scd=none,`
      +`trim=start_frame=${firstFrame}:end_frame=${firstFrame+forwardCount},setpts=N/(${fps}*TB)`,
      '-an','-c:v','ffv1',forward]);
    const filter='[0:v]split=2[f][back];'
      +`[back]trim=start_frame=1:end_frame=${forwardCount-1},reverse,setpts=N/(${fps}*TB)[b];`
...
console.log(`Exported ${loopSeconds}-second, ${fps} fps forward/reverse photo loops without held frames.`);
